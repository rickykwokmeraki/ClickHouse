...
def test_install(image: DockerImage, tests: Dict[str, str]) -> List[Result]:
    ...
    for name, command in tests.items():
        run_command = (
            f"docker run --rm --privileged --cap-add=SYS_PTRACE "
            f"--volume={TEMP_PATH}:/packages {image} /packages/install.sh"
        )
        print(f"Running docker container: [{run_command}]")
        # container_id = Shell.ch(run_command, verbose=True, strict=True)
        (TEMP_PATH / "install.sh").write_text(command)
        install_command = f"docker exec install_check bash -ex /packages/install.sh"
